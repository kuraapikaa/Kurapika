      ...
      <div className="space-y-5 animate-in fade-in duration-300">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <button type="button" onClick={() => setSelectedBTag(null)} className="rounded-xl border border-white/10 bg-white/5 p-2 text-slate-400 hover:text-white" title="Geri dön">
              <ChevronLeft size={18} />
            </button>
            <div>
        ...
        <div className="overflow-hidden rounded-xl border border-white/10 bg-[#0d1119]">
          {selectedBTag === 'BTag Yok' ? (
            ...
            <div className="p-10 text-center text-sm text-slate-500">Oyuncular yükleniyor...</div>
          ) : players.length === 0 ? (
            <div className="p-10 text-center text-sm text-slate-500">Bu BTag için oyuncu bulunamadı.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full min-w-[900px] text-left text-xs">
                <thead className="border-b border-white/5 bg-black/20 text-[10px] uppercase tracking-wider text-slate-500">
                  <tr>
                  ...
                  {players.map((player: ClientItem) => (
                    <tr key={player.Id} className="hover:bg-white/[0.03]">
                      <td className="px-5 py-4">
                        ...
                        <div className="mt-1 text-[10px] text-slate-600">#{player.Id}</div>
                      </td>
    ...
    <div className="space-y-5 animate-in fade-in duration-300">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <label className="relative block w-full sm:w-72">
          ...
          <input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="BTag ara..." className="h-10 w-full rounded-xl border border-white/10 bg-[#0d1119] pl-10 pr-3 text-sm text-white outline-none focus:border-cyan-400/40" />
        </label>
      </div>
          ...
          <table className="w-full min-w-[760px] text-left text-xs">
            ...
            <tbody className="divide-y divide-white/5">
              {summaryQuery.isLoading ? (
                <tr><td colSpan={5} className="px-5 py-14 text-center text-sm text-slate-500">BTag raporu hazırlanıyor; oyuncu sayısı rapordan dinamik hesaplanıyor...</td></tr>
              ) : summaryQuery.error ? (
                ...
                <tr><td colSpan={5} className="px-5 py-14 text-center text-sm text-slate-500">BTag kaydı bulunamadı.</td></tr>
