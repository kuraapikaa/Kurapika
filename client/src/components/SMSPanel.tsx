            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-semibold italic tracking-tighter text-white uppercase">
                        SMS <span className="text-purple-400">MERKEZİ</span>
                    </h1>
            ...
            <AnimatePresence mode="wait">
                {activeTab === 'send' && (
                    ...
                    >
                                ...
                                <CardHeader>
                                    <div className="flex items-center justify-between">
                                        <CardTitle className="flex items-center gap-2">
                                            <Users className="w-4 h-4 text-purple-400" /> ALICILAR
                                        </CardTitle>
                                        <button
                            ...
                            <Card>
                                <CardHeader>
                                    <CardTitle className="flex items-center gap-2 text-white">
                                        <MessageSquare className="w-4 h-4 text-purple-400" /> MESAJ İÇERİĞİ
                                    </CardTitle>
                                </CardHeader>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {['[NAME]', '[URL]', '[DATE]'].map(tag => (
                                            <button
                                                ...
                                                className="px-3 py-1 bg-white/5 hover:bg-white/10 border border-white/5 rounded-lg text-xs text-slate-400 transition-colors"
                                            >
                                                {tag}
                                            </button>
                                        ))}
                        ...
                        {/* Sağ: Bilgi & Önizleme */}
                        <div className="space-y-6">
                            <Card className="bg-gradient-to-br from-blue-600/10 to-transparent border-purple-400/25">
                                <CardHeader>
                                    <CardTitle className="flex items-center gap-2">
                                        <Info className="w-4 h-4 text-purple-400" /> ÖZET BİLGİ
                                    </CardTitle>
                                </CardHeader>
                                <CardContent className="space-y-4">
                            ...
                            <div className="relative group">
                                ...
                                <div className="relative bg-white/[0.02] border-8 border-white/[0.05] rounded-3xl p-8 shadow-2xl min-h-[400px] backdrop-blur-xl">
                                    ...
                                    <div className="flex items-center gap-3 mb-6">
                                        ...
                                        <div>
                                            <div className="text-[10px] font-bold text-slate-400 uppercase">GÖNDEREN</div>
                ...
                {activeTab === 'templates' && (
                    ...
                    >
                            ...
                            <Card key={tmpl.id} className="group hover:border-blue-500/50 transition-all cursor-pointer overflow-hidden" onClick={() => applyTemplate(tmpl.content)}>
                                <CardHeader className="bg-white/5 border-b border-white/5">
                                    <div className="flex justify-between items-start">
                                        <div className="p-2 bg-blue-600/20 rounded-lg text-purple-300">
                        ...
                        <Card className="border-dashed border-white/5 bg-transparent flex flex-center hover:bg-white/5 transition-all cursor-pointer min-h-[200px]">
                            <div className="m-auto text-center p-6">
                                ...
                                <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider">Özel Şablon Ekle</h3>
                                <p className="text-xs text-slate-500 mt-2">Yakında aktif olacak</p>
                            </div>
                        </Card>
                    </motion.div>
                )}
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="bg-white/5">
                                            <th className="p-4 text-xs font-semibold text-slate-400 uppercase tracking-[0.2em] border-b border-white/5">ZAMAN</th>
                                            <th className="p-4 text-xs font-semibold text-slate-400 uppercase tracking-[0.2em] border-b border-white/5">MESAJ</th>
                                            <th className="p-4 text-xs font-semibold text-slate-400 uppercase tracking-[0.2em] border-b border-white/5">ALICI</th>
                                            <th className="p-4 text-xs font-semibold text-slate-400 uppercase tracking-[0.2em] border-b border-white/5">DURUM</th>
                                            <th className="p-4 text-xs font-semibold text-slate-400 uppercase tracking-[0.2em] border-b border-white/5"></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {history.length === 0 ? (
                                            <tr>
                                                <td colSpan={5} className="p-12 text-center">
                                                    ...
                                                    <p className="text-slate-400 font-medium">Henüz bir gönderim geçmişi bulunmuyor.</p>
                                                </td>
                                            </tr>
                                        ) : (
                                            history.map((log) => (
                                                <tr key={log.id} className="hover:bg-white/5 transition-colors border-b border-white/5">
                                                    ...
                                                    <td className="p-4 max-w-xs">
                                                        <p className="text-xs text-slate-400 line-clamp-1 italic">"{log.message}"</p>
                                                    </td>
                                                    <td className="p-4">
                                                        <div className="flex items-center gap-2">
                                                            <Users className="w-3 h-3 text-slate-400" />
                                                            <span className="text-sm font-bold text-slate-300">{log.recipientCount}</span>
                                                        </div>
                                                    </td>
                                                    <td className="p-4">
                                                        {log.status === 'success' ? (
