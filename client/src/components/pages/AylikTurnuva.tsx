...
export function AylikTurnuva() {
  ...
  const [prize, setPrize] = useState('500.000');
  ...
  const fetchData = async () => {
    ...
    try {
      tournamentApi.getSettings().then((settings) => {
        if (settings?.aylik?.prize) setPrize(settings.aylik.prize);
      }).catch(() => {});
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center overflow-x-hidden bg-[#05060a] pb-8 font-sans text-zinc-200">
      ...
      <div className="relative z-10 w-full max-w-6xl px-3 py-5 sm:px-4 sm:py-8 md:px-8">
        <TournamentPeriodSwitch active="aylik" />

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative mb-5 w-full overflow-hidden rounded-[2rem] border border-white/5 p-5 sm:mb-10 sm:p-8 md:rounded-[3rem] md:p-14">
          <div className="absolute inset-0 bg-gradient-to-br from-purple-900/40 via-zinc-950 to-zinc-950 opacity-50" />
          <div className="relative z-10 flex flex-col items-center justify-between gap-6 md:flex-row md:gap-10">
            <div className="max-w-xl text-center md:text-left">
              <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/5 bg-white/5 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-zinc-200 sm:mb-6">
                <Trophy size={14} className="text-purple-400" /> Şampiyonlar ligi
              </div>
              <h1 className="mb-2 text-4xl font-black uppercase leading-[0.95] tracking-tighter text-white sm:mb-6 sm:text-5xl md:text-8xl">
                <span className="bg-gradient-to-r from-purple-400 via-fuchsia-400 to-purple-500 bg-clip-text text-transparent">{prize}₺</span> <br />
                Ödül havuzu
              </h1>
            </div>
            <div className="relative hidden sm:block">
              <div className="absolute inset-0 animate-pulse rounded-full bg-purple-600/40 blur-[60px]" />
        ...
        <MobileLeaderboard data={data} loading={loading} periodLabel="Aylık turnuva" />
