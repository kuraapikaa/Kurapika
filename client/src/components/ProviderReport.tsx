                    ...
                    <table className="w-full text-sm text-left">
                        <thead>
                            <tr className="border-b border-white/5 text-[10px] font-black uppercase tracking-widest text-slate-500">
                                <th className="px-6 py-4">Sağlayıcı</th>
                                <th className="px-6 py-4 text-right">Ciro (Bet)</th>
                                <th className="px-6 py-4 text-right">Ödenen (Win)</th>
                                <th className="px-6 py-4 text-right">Karlılık (NGR)</th>
                                <th className="px-6 py-4 text-right">Royalty (%{royaltyRate})</th>
                                <th className="px-6 py-4 text-right">RTP (%)</th>
                            </tr>
                        </thead>
                            ...
                            {providers.map((item, idx) => {
                                ...
                                return (
                                    <tr key={idx} className="group hover:bg-white/[0.02] transition-colors">
                                        ...
                                        <td className={`px-6 py-4 text-right font-mono font-black ${isProfit ? 'text-emerald-400' : 'text-rose-400'}`}>
...
function StatCard({ label, value, icon, trend }: { label: string; value: number; icon: React.ReactNode; trend: 'up' | 'down' | 'neutral' }) {
    return (
        <div className="rounded-2xl border border-white/5 bg-slate-900/50 p-5 shadow-lg group hover:border-white/10 transition-all">
