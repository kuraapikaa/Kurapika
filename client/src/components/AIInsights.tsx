    ...
    if (isLoading) {
        return (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-pulse mt-8">
                {[1, 2].map(i => (
                    <div key={i} className="h-48 rounded-3xl bg-zinc-900/50 border border-white/5" />
                ))}
            </div>
        );
    }
        ...
        <div className="space-y-8 mt-8">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-xl font-black text-white uppercase tracking-tight">İşletme Analitikleri</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                    <div className="flex items-center gap-2 px-1">
                        ...
                        <span className="text-[10px] font-black text-amber-400 uppercase tracking-widest">Kritik Analizler</span>
                    </div>
                    {insights.length === 0 ? (
                            ...
                            <Card className={cn(
                                ...
                                <div className="flex items-start gap-4">
                                    ...
                                    <div className="flex-1 space-y-2">
                                        <div className="flex items-center justify-between">
                                            <h4 className="text-sm font-black text-white uppercase tracking-tight antialiased">{insight.title}</h4>
                                            {insight.metric && (
                                                <span className="text-[10px] font-black px-2 py-0.5 rounded-lg bg-white/10 text-white/80 uppercase tracking-tighter">{insight.metric}</span>
                                            )}
                                        </div>
                ...
                <div className="space-y-6">
                            ...
                            <Card className="p-6 bg-gradient-to-br from-blue-500/5 to-teal-500/5 border-white/5 group hover:border-blue-500/20 transition-all duration-300 shadow-xl backdrop-blur-sm">
                                <div className="flex items-center justify-between mb-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h4 className="text-sm font-black text-white uppercase tracking-tight">{bonus.bonusName}</h4>
                                            <p className="text-[10px] font-bold text-zinc-500 uppercase">{bonus.targetGroup}</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-[10px] font-black text-emerald-400 uppercase tracking-widest">Tahmini Artış</p>
                                        <p className="text-lg font-black text-white">+{bonus.estimatedCvr}%</p>
                                    </div>
                                </div>
                                ...
                                <button className="w-full mt-4 flex items-center justify-center gap-2 py-3 rounded-xl bg-white text-zinc-950 text-[10px] font-black uppercase tracking-widest hover:bg-zinc-200 transition-all group/btn shadow-lg">
                    ...
                    <Card className="relative overflow-hidden p-6 bg-zinc-900/20 border-dashed border-white/10 flex flex-col items-center justify-center text-center space-y-4 py-10 group">
                        ...
                        <div className="relative z-10 flex flex-col items-center space-y-4">
                            ...
                            <div className="space-y-1">
                                <p className="text-xs font-black text-white uppercase tracking-[0.2em] animate-pulse">Analiz Motoru Aktif</p>
                            ...
                            <div className="flex flex-wrap justify-center gap-2 max-w-[280px]">
                                {['Syncing KPI', 'Pattern Match', 'GGR Audit'].map((tag, idx) => (
                                    <span key={idx} className="text-[8px] font-black px-2 py-0.5 rounded-full bg-white/5 border border-white/5 text-zinc-500 uppercase tracking-tighter">
                                        {tag}
                                    </span>
                                ))}
                            </div>
