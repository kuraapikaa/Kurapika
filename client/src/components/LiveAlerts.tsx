                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                            ...
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Anlık Takip</p>
                        </div>
                    </div>
                </div>
                <div className="px-2 py-1 rounded-full bg-white/5 border border-white/5">
                    <span className="text-[9px] font-semibold text-slate-400 uppercase tracking-tighter">Sistem Durumu: KRİTİK</span>
                </div>
            </div>
            ...
            <div className="space-y-3 max-h-[480px] overflow-y-auto overflow-x-hidden pr-3 scrollbar-hide">
                <AnimatePresence mode="popLayout">
                    {anomalies.length === 0 ? (
                        ...
                        >
                            <ShieldAlert size={32} className="mx-auto text-slate-500 mb-3" />
                            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Analiz edilecek veri bekleniyor...</p>
                        </motion.div>
                    ) : (
                        anomalies.slice(0, 10).map((alert: any, i: number) => (
                            ...
                            >
                                <div className="flex items-start justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">{alert.type?.replace(/_/g, ' ')}</span>
                                        </div>
                                    </div>
                                    {alert.clientId && (
                                        <Link
                                ...
                                <p className="mt-3 text-[11px] leading-relaxed text-slate-300 font-medium tracking-tight">
                                    {alert.message}
                                </p>

                                <div className="mt-3 pt-3 border-t border-white/5 flex items-center justify-between">
                                    <span className="text-[8px] font-semibold text-slate-400 uppercase tracking-[0.1em]">{alert.date}</span>
                                    <div className="flex items-center gap-1.5">
                                        <div className={cn("w-1.5 h-1.5 rounded-full", alert.severity === 'high' ? "bg-rose-500" : "bg-amber-500")} />
                                        <span className="text-[8px] font-semibold uppercase text-slate-400">{alert.severity === 'high' ? 'TEHLİKELİ' : 'DİKKAT'}</span>
                                    </div>
                                </div>
                            </motion.div>
                        ))
