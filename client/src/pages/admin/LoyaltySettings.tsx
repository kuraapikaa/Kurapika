...
export function LoyaltySettings() {
    ...
    if (marketLoading) return <div className="p-20 text-center"><RefreshCw className="animate-spin mx-auto text-purple-400" /></div>;
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex items-center justify-between">
                <div>
                   ...
                   <p className="text-slate-400 text-sm font-bold mt-1">Puan kazanım oranlarını ve ödül marketini yönetin.</p>
                </div>
                <button 
            ...
            <div className="p-8 bg-white/[0.02] border border-white/5 rounded-xl">
                <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                        ...
                        <p className="text-slate-400 text-xs font-bold">Her 1 puan için gereken toplam bahis (turnover) miktarı.</p>
                    </div>
                </div>
                <div className="flex items-center gap-4 max-w-sm">
                   <div className="flex-1 space-y-2">
                        <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest px-1">BAHİS MİKTARI (TL)</label>
                        <input 
                            type="number" 
                            value={wagerRatio} 
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between px-2">
                    <h2 className="text-sm font-semibold text-white uppercase tracking-[0.2em] flex items-center gap-3">
                        <Store size={18} className="text-purple-400" /> ÖDÜL MARKETİ ÜRÜNLERİ
                    </h2>
                    <button 
                    ...
                    {market.map((item, idx) => (
                        <div key={item.id} className="p-6 bg-white/[0.02] border border-white/5 rounded-xl space-y-4 group hover:border-amber-500/20 transition-all">
                             <div className="flex items-center justify-between">
                                ...
                                <button onClick={() => removeMarketItem(item.id)} className="p-2 text-slate-500 hover:text-rose-500 transition-colors">
                                    <Trash2 size={18} />
                                </button>
                             </div>
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                ...
                                <div className="space-y-1 col-span-3">
                                    <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest">ÖDÜL TİPİ</label>
                                    <select 
                                        value={item.rewardType} 
                                        onChange={e => { const newM = [...market]; newM[idx].rewardType = e.target.value as any; setMarket(newM); }} 
                                        className="w-full bg-white/[0.02] border border-white/5 rounded-xl px-3 py-2 text-[10px] font-semibold text-white outline-none focus:border-amber-500/50 appearance-none cursor-pointer pr-4 bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20fill%3D%22none%22%20viewBox%3D%220%200%2020%2020%22%3E%3Cpath%20stroke%3D%22%23fbbf24%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%20stroke-width%3D%221.5%22%20d%3D%22m6%208%204%204%204-4%22%2F%3E%3C%2Fsvg%3E')] bg-[position:right_0.5rem_center] bg-[length:1.2em_1.2em] bg-no-repeat"
                                    >
                                        <option value="freespin" className="bg-white/[0.02] text-white">Freespin</option>
                                        <option value="cash" className="bg-white/[0.02] text-white">Nakit Para</option>
                                        <option value="bonus" className="bg-white/[0.02] text-white">Özel Bonus</option>
                                    </select>
                                </div>
                            </div>
                        </div>
