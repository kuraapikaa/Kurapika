        ...
        <div className="p-5 space-y-4">
          ...
          <div className="flex gap-2 justify-end">
            ...
            <button
              onClick={() => onSave(note)}
              disabled={isSaving}
              className="px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-[#050609] text-sm font-semibold flex items-center gap-2 transition-colors disabled:opacity-50"
            >
    ...
    <div className="mx-auto max-w-7xl space-y-6 p-4 md:p-6 pb-24">
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className={cn('px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 transition-all', activeTab === 'call' ? 'bg-sky-500 text-[#050609]' : 'bg-white/5 text-slate-400 hover:text-white')}
        >
        ...
        <button
          ...
          className={cn('px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 transition-all', activeTab === 'partnership' ? 'bg-blue-500 text-white' : 'bg-white/5 text-slate-400 hover:text-white')}
        >
          <Handshake size={15} /> Ortaklık
          {partnershipStats.pending > 0 && (
            <span className={cn('rounded-full text-xs font-semibold px-1.5 py-0.5 min-w-[20px] text-center', activeTab === 'partnership' ? 'bg-black/20 text-white' : 'bg-purple-400/20 text-purple-300')}>
        ...
        <button
          ...
          className={cn('px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 transition-all', activeTab === 'settings' ? 'bg-[color:var(--panel-text,#f2f4f8)] text-[#050609]' : 'bg-white/5 text-slate-400 hover:text-white')}
        >
              ...
              <div className="flex gap-1 bg-white/5 border border-white/5 rounded-xl p-1">
                {(['all', 'pending', 'completed'] as const).map(s => (
                  <button
                    ...
                    className={cn('px-3 py-1 rounded-md text-xs font-bold transition-all', statusFilter === s ? 'bg-white/10 text-white' : 'text-slate-400 hover:text-white')}
                  >
            ...
            {activeTab === 'call' && (
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm text-slate-400">
                  <thead className="bg-black/20 text-xs uppercase font-semibold text-slate-400">
                    <tr>
                  ...
                  <tbody className="divide-y divide-white/[0.04]">
                    {filteredCalls.map((r: any) => (
                      <tr key={r.id} className="hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="px-5 py-3.5 text-slate-200 max-w-[160px] truncate" title={r.reason}>{r.reason}</td>
                        <td className="px-5 py-3.5">
                          <button
                            onClick={() => setNoteModal({ collection: 'call', id: r.id, note: r.note || '', status: r.status })}
                            className={cn(
                              'flex items-center gap-1.5 px-2.5 py-1 rounded-xl text-xs font-bold transition-colors',
                              r.note ? 'bg-amber-500/15 text-amber-400 hover:bg-amber-500/25' : 'bg-white/5 text-slate-500 hover:text-slate-200 hover:bg-white/10'
                            )}
                          ...
                          <div className="flex items-center justify-end gap-2">
                            {r.status !== 'completed' && (
                              <button
                                ...
                                className="bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500 hover:text-[#050609] px-3 py-1.5 rounded-xl text-xs font-bold transition-colors whitespace-nowrap"
                              >
                                İlgilenildi
                              </button>
                            )}
                  ...
                  <tbody className="divide-y divide-white/[0.04]">
                    {filteredPartnerships.map((r: any) => (
                      <tr key={r.id} className="hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="px-5 py-3.5">
                          <div className="font-bold text-purple-300 text-xs">{r.type}</div>
                        ...
                        <td className="px-5 py-3.5">
                          ...
                          {r.audienceSize && <div className="text-xs text-slate-400 mt-0.5">Kitle: {r.audienceSize}</div>}
                        </td>
                        <td className="px-5 py-3.5 max-w-[140px] truncate text-xs" title={r.message}>{r.message || <span className="text-slate-500">—</span>}</td>
                        <td className="px-5 py-3.5">
                          <button
        ...
        <div className="bg-white/[0.02] border border-white/5 rounded-xl p-6 space-y-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-5">
              <div className="flex items-center justify-between pb-3 border-b border-white/5">
                ...
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  ...
                  <span className="text-sm font-bold text-slate-400">{callActive ? 'Aktif' : 'Pasif'}</span>
                </label>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="text-xs text-slate-400 mb-1 block">Başarı Mesajı</label>
                    <input
                      type="text"
                ...
                {callReasons.map((reason, idx) => (
                  <div key={idx} className="flex gap-2">
                    ...
                    <button
                      ...
                      className="px-3 bg-rose-500/10 text-rose-500 rounded-xl hover:bg-rose-500 hover:text-white transition-colors"
                    >
                      <Trash2 size={15} />
                    </button>
                  </div>
            ...
            <div className="space-y-5">
              <div className="flex items-center justify-between pb-3 border-b border-white/5">
                <h3 className="text-base font-semibold text-white flex items-center gap-2">
                  <Handshake size={17} className="text-purple-300" /> Ortaklık Formu
                </h3>
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  <div
                  ...
                  <span className="text-sm font-bold text-slate-400">{partnershipActive ? 'Aktif' : 'Pasif'}</span>
                </label>
              </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 py-2 rounded-xl bg-white/5 text-slate-400 hover:text-white text-sm font-bold transition-colors"
                >
                  İptal
                </button>
                <button
                  ...
                  className="flex-1 py-2 rounded-xl bg-rose-500 hover:bg-rose-400 text-white text-sm font-semibold flex items-center justify-center gap-2 transition-colors disabled:opacity-50"
                >
