...
import { WithdrawalChecklistModal } from '@/components/admin/WithdrawalChecklistModal';
import { formatNumber, formatDateTimeWithSeconds } from '@/lib/format';
import { useDateRange } from '@/context/DateRangeContext';
import { AdminCard, StatusBadge as Chip } from '@/components/ui/admin';
...
function StatusBadge({ stateName }: { stateName?: string | null }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full bg-[rgba(242,244,248,0.08)] px-2.5 py-1 text-[10px] font-bold text-slate-400 ring-1 ring-slate-500/20">
      {stateName || '—'}
    </span>
  );
}
...
const STAT_TONE = {
  ...
  neutral: 'border-white/5 bg-white/[0.04] text-slate-200',
} as const;
  ...
  return (
    <AdminCard className="flex items-center gap-3 p-3.5">
      ...
      <div className="min-w-0">
        ...
        <p className="mt-1 text-[11px] text-slate-400">{count} işlem</p>
      </div>
    </AdminCard>
  );
}
  ...
  return (
    <div className="animate-in space-y-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex min-w-0 flex-1 items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-[12px] text-slate-400">Çekim talepleri · tarih aralığına göre</p>
      ...
      {loadingClientId !== null && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center gap-5 bg-white/[0.02] backdrop-blur-md">
          <div className="relative">
            <div className="absolute inset-0 animate-ping rounded-full bg-purple-400/20" />
            <Loader2 size={56} className="relative animate-spin text-purple-300" />
          </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-white/5 bg-white/[0.02] shadow-xl transition-shadow hover:shadow-2xl hover:shadow-blue-500/5">
          <div className="flex flex-col border-b border-white/5 bg-white/5 sm:flex-row sm:items-center sm:justify-between px-3 py-2.5 gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="text-[10px] font-bold text-slate-400 ml-2">
                Gösterilen: <span className="text-slate-200">{filteredRequests.length}</span>
              </div>
            </div>
          </div>
          ...
          {withdrawalRequestsQuery.isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-4 py-20">
              <div className="flex h-20 w-20 items-center justify-center rounded-xl bg-white/10 text-slate-400">
                <Inbox size={40} />
              </div>
              <p className="font-bold text-slate-400">Aramanıza veya filtrenize uygun talep bulunamadı.</p>
              <p className="text-xs text-slate-400">Arama metnini değiştirmeyi veya farklı bir aralık seçmeyi deneyin.</p>
            </div>
          ) : (
            <div className="flex flex-col">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/5 text-left text-[10px] font-extrabold uppercase tracking-widest text-slate-400">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {paginatedRequests.map((row: WithdrawalRequestItem, index: number) => (
                      ...
                      >
                        <td className="px-5 py-4">
                          ...
                          {row.ClientLogin ? (
                            <Link
                              ...
                              className="text-xs text-slate-400 transition-colors hover:text-purple-300 block w-fit mt-0.5"
                            >
                              {row.ClientLogin}
                            </Link>
                          ) : (
                        ...
                        <td className="px-5 py-4 font-mono text-base font-bold tabular-nums text-slate-200">
                          {formatNumber(row.Amount)} <span className="text-xs font-normal text-slate-400">{row.CurrencyId || 'TRY'}</span>
                        </td>
                        <td className="px-5 py-4">
              ...
              {/* Sayfalama */}
              {totalPages > 1 && (
                <div className="flex items-center justify-between border-t border-white/5 bg-white/[0.02] px-3 py-2.5">
