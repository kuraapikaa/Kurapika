...
Lynon entegrasyonundaki rapor, oyuncu, finans ve API eşleme modülleri yapılandırmayı
...
| Dosya | Neden önemli |
|---|---|
| [`services/lynonBackofficeService.ts`](server/src/services/lynonBackofficeService.ts) | Geriye uyumlu Lynon cephesi ve çapraz-akış orkestrasyonu |
| [`services/lynon/raporlar.ts`](server/src/services/lynon/raporlar.ts) | Rapor kataloğu, 1838–1848 kimlikleri ve operasyon KPI çağrıları |
| [`services/lynon/oyuncu.ts`](server/src/services/lynon/oyuncu.ts) | Oyuncu arama, KPI, hesap, kısıt ve bakiye işlemleri |
| [`services/lynon/finans.ts`](server/src/services/lynon/finans.ts) | Yatırım, çekim, finansal hareket ve işlem geçmişi |
| [`services/lynon/apiEsleme.ts`](server/src/services/lynon/apiEsleme.ts) | Lynon istek/yanıt alanlarını panel modellerine eşleyen saf yardımcılar |
...
**Lynon modülerleşmesi.** Rapor, oyuncu, finans ve API eşleme sorumlulukları
`services/lynon/` altına ayrıldı. Eski importları kırmamak için
`lynonBackofficeService.ts` geriye uyumlu cephe olarak duruyor; kalan bonus,
mutabakat ve kampanya orkestrasyonu da aynı desenle inceltilebilir.
