...
export interface HuniAsamasi {
  etiket: string;
  /** `null` = bu aşama bu dönemde ölçülmüyor; sıfır İLE KARIŞTIRILMAZ. */
  deger: number | null;
}

/**
 * HUNİ — aşamalı dönüşüm (tıklama → kayıt → ilk yatırım gibi).
 *
 * Her çubuk bir önceki aşamaya değil İLK aşamaya (tavan) göre
 * ölçekleniyor; böylece "bu aşama baştaki hacmin ne kadarı" tek
 * bakışta okunuyor. Aşamalar arası dönüşüm yüzdesi ayrıca yazılıyor —
 * o, çubuk genişliğinden BAŞKA bir soru ("bir önceki aşamanın ne
 * kadarı buraya geçti").
 *
 * `null` değer sıfır genişlikte bir çubuk DEĞİL: ölçülmeyen bir aşama
 * (örn. kalibrasyon süren ilk yatırım) kesik çizgili bir kutu ve
 * "Ölçülmüyor" yazısıyla gösteriliyor — aksi halde "bu aşamada kimse
 * yok" ile "bu aşamayı ölçemiyoruz" ayrımı kaybolurdu.
 */
export function Huni({ asamalar }: { asamalar: HuniAsamasi[] }) {
  const tavan = Math.max(1, ...asamalar.map((a) => a.deger ?? 0));

  return (
    <div className="space-y-4">
      {asamalar.map((asama, i) => {
        const onceki = i > 0 ? asamalar[i - 1] : null;
        const donusum = onceki?.deger && asama.deger !== null
          ? Math.round((asama.deger / onceki.deger) * 1000) / 10
          : null;
        const genislik = asama.deger !== null ? Math.max(3, (asama.deger / tavan) * 100) : 100;

        return (
          <div key={asama.etiket}>
            <div className="mb-1.5 flex items-baseline justify-between gap-3 text-sm">
              <span className="font-medium">{asama.etiket}</span>
              <span className="flex items-baseline gap-2">
                ...
                <span className="tabular-nums font-semibold">
                  {asama.deger === null ? '—' : bicim(asama.deger)}
                </span>
              </span>
            </div>
            {asama.deger === null ? (
              <div
                className="flex h-8 items-center rounded-lg border border-dashed px-3 text-xs"
                style={{ borderColor: 'var(--kenar)', color: 'var(--metin-2)' }}
              >
                Ölçülmüyor
              </div>
            ) : (
              <div className="h-8 overflow-hidden rounded-lg" style={{ background: 'var(--yuzey-2)' }}>
                <div
                  className="h-full rounded-lg"
                  style={{ width: `${genislik}%`, background: 'var(--vurgu)' }}
                />
              </div>
            )}
          </div>
