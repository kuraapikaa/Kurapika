 ...
 *
 * Huni `BarList` ile çiziliyor, özel bir SVG bileşeniyle değil —
 * Tremor'un zaten sahip olduğu bir bileşeni tekrar icat etmek yalnızca
 * bakım yükü ekler. İlk yatırım ölçülmüyorsa o satır listeye hiç
 * EKLENMİYOR (sıfır değer vermek yerine): sıfır "kimse yapmadı" der,
 * ölçülemedi "bilmiyoruz" der — ikisi çok farklı iddialar.
 */
  ...
  return (
    <>
      <ShadCard>
        ...
        <CardContent>
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <label className="block">
              <span className="mb-1.5 block text-xs font-medium text-muted-foreground">Ortak anahtarı</span>
              <Input value={taslak.ortakAnahtari} onChange={(e) => setTaslak((t) => ({ ...t, ortakAnahtari: e.target.value }))} />
              <p className="mt-1 text-xs text-muted-foreground">Boş bırakılırsa tüm ortaklar birlikte gösterilir.</p>
            </label>
          </div>
        </CardContent>
      </ShadCard>
      ...
      {yukleniyor ? (
        ...
        <ShadCard><CardContent className="pt-6"><BosDurum mesaj="Bu aralıkta tıklama ya da kayıt yok." /></CardContent></ShadCard>
      ) : (
        <>
          ...
          <ShadCard>
            <CardHeader><CardTitle className="text-sm font-medium text-muted-foreground">Trafik nereden geliyor</CardTitle></CardHeader>
            <CardContent>
